...
# =============================================================================
# Lab 5 — Query Pesada Controlada: Setup
# Cria o índice lab5-produtos e indexa dataset-large.json (1000 docs)
# =============================================================================
...
DATASET_PATH="${SCRIPT_DIR}/../dataset/dataset-large.json"
MIN_DOCS=1000
...
index_dataset() {
  if [ ! -f "$DATASET_PATH" ]; then
    ...
    error "Certifique-se de que o arquivo modulo6-lab/dataset/dataset-large.json existe."
    exit 1
  fi

  local total
  total=$(awk 'NR%2==0' "$DATASET_PATH" | wc -l | tr -d ' ')
  log "Indexando ${total} documentos via bulk API (dataset-large.json)..."

  # Substitui _index pelo nome correto
  local bulk_data
...
if index_exists; then
  warning "Índice '${INDEX_NAME}' já existe. Verificando volume de documentos..."
  ...
  if [ "${doc_count}" -ge "${MIN_DOCS}" ] 2>/dev/null; then
    success "Índice '${INDEX_NAME}' já está pronto com ${doc_count} documentos (mínimo: ${MIN_DOCS})."
  else
    warning "Volume insuficiente (${doc_count} docs). Indexando dataset-large.json automaticamente..."
    index_dataset
  fi
else
  create_index
...
echo ""
log "Próximos passos:"
log "  ./query-wildcard-agg.sh  — executa wildcard + aggregation (query custosa)"
log "  ./query-otimizada.sh     — executa term + aggregation (query otimizada)"
echo ""
