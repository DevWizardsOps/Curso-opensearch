...
cmd_connect() {
  ...
  local aluno_num
  aluno_num=$(echo "$aluno_id" | grep -oE '[0-9]+
    --output text 2>/dev/null)
...
case $COMMAND in
 ...
 || echo "")
  ...
  # Remover zeros à esquerda para lookup no CloudFormation (output keys são EC2Aluno1IP, EC2Aluno2IP, etc.)
  aluno_num=$((10#${aluno_num}))
